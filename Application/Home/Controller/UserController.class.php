...
class UserController extends PSIBaseController {
	...
	public function changeMyPassword() {
		...
		if($loginType == FIdConst::LOGIN_TYPE_EMPLOYEE){
			if (FIdConst::CHANGE_MY_PASSWORD4EMPLOYEE) {
				$this->initVar4Employee();
				$employeeInfos = M("info_employee")->where('id='.session('loginUserId'))->select();
				if(count($employeeInfos)>0){
					$employeeInfo = $employeeInfos[0];
					$this->assign("loginUserId", $us->getLoginUserId());
					$this->assign("loginName", $employeeInfo['client_user_name']);
					$this->assign("loginUserName", $employeeInfo['name']);
		...
		}elseif($loginType == FIdConst::LOGIN_TYPE_STAFF){
     ...
     * 获得某个人员的业务分配的医院分配信息
     */
    public function yeWuSetHospitalAssignList() {
        if (IS_POST) {
            $us = new UserService();
            $params = array(
            ...
            $data = $us->yeWuSetHospitalAssignList($params);
     ...
     * 新增或编辑某个人的分配的业务信息
     */
    public function editYeWuSet4HospitalAssign() {
        if (IS_POST) {
            // 检查权限
            $us = new UserService();
            if (I("post.user_id")) {
                ...
                if (! $us->hasPermission(FIdConst::USER_MANAGEMENT_EDIT_USER)) {
                    $this->ajaxReturn($this->noPermission("编辑用户业务配置"));
                    return;
                }
            } else {
                    $this->ajaxReturn($this->noPermission("编辑用户业务配置"));
            ...
            $params = array(
                ...
                "hospitalAssignIdList" => I("post.hospitalAssignIdList"),
                "drugIdList" => I("post.drugIdList"),
                "dataOrgList" => I("post.dataOrgList")
            );

            $result = $us->editYeWuSet4HospitalAssign($params);
	...
	public function loginPOST() {
		if (IS_POST) {
			...
			$us = new UserService();
			$this->ajaxReturn($us->doLogin($loginName, $password, $fromDevice,$loginType));
		}
	}
