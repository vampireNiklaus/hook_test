 ...
 * @author RCG
 */
class SelfPayByTwoService extends PSIBaseService {
    private $LOG_CATEGORY = "业务流程信息-付款单信息";

    public function editSelfPayByTwo($params,$isParent,$isFund){
        //编辑付款单
        if ($this->isNotOnline()) {
            return $this->emptyResult();
        }
        $selfPay_db = M('bill_self_pay_by_two');
        if($isFund){
            //只要修改打款日期跟note
    ...
    public function listSelfPayByTwoUnEdit($params){
        if ($this->isNotOnline()) {
            return $this->emptyResult();
        }
        $selfPay_db = M('bill_self_pay_by_two');
...
//        $whereStr = $this->likeSearch($params);
        ...
        $all_data = $selfPay_db
            ->alias('sp')
            ->join('info_drug AS idr ON idr.id=sp.drug_id')
            ->join('bill_self_purchase_by_two AS spu ON spu.id=sp.buy_bill_id')
            ->where("sp.status=0 AND idr.common_name like '%".$params['common_name']."%' AND sp.bill_code like '%".$params['bill_code']."%'")
            ->field('sp.*,idr.common_name,idr.jx,idr.goods_name,idr.guige,idr.manufacturer,spu.bill_code buy_bill_code')
            ->page($page,$limit)
            ->select();
    ...
    public function listSelfPayByTwoEdit($params){
        if ($this->isNotOnline()) {
            return $this->emptyResult();
        }
        $selfPay_db = M('bill_self_pay_by_two');
...
//        $whereStr = $this->likeSearch($params);
        ...
        $all_data = $selfPay_db
            ...
            ->join('LEFT JOIN info_bank_account AS iba2 ON iba2.id=sp.pay_2nd_account')
            ->where($where)
            ->field('sp.*,idr.common_name,idr.jx,idr.goods_name,idr.guige,idr.manufacturer,
     ...
     * *
     */
    public function deleteSelfPayByTwo($params){
        if ($this->isNotOnline()) {
            return $this->notOnlineError();
        }

        $db = M("bill_self_pay_by_two");
        ...
        if($data['status']==0||$data['status']==2){
            //设置对应采购单的状态为未通过
            M('bill_self_purchase_by_two')->where('id='.$data['buy_bill_id'])->save(array('status'=>2));
...
//                            $this->createSelfStockKaiPiao($id);
                ...
                switch ($statusNow){
                    ...
                    case FIdConst::SELF_PAY_STATUS_VERIFY_PASSED:
                        ...
                        if($this->beforeVerifyReturn($id)){
                            ...
                            $data['status']=FIdConst::SELF_PAY_STATUS_VERIFY_DENIED;
                            //删除对应的入商业公司配送单
                            M("bill_self_stock_by_two")->where("pay_bill_id=".$id)->delete();
                            //资金回流
    ...
    public function beforeVerifyReturn($id){
        //反审核之前检查是否已经入商业库，如果已存在入库，则不能反审核
        $Stock = M("bill_self_stock_by_two")->where("pay_bill_id=".$id)->find();
    ...
    public function accountPayMoney($id){
        ...
        try{
            ...
            if($account_1_can){
                ...
                if($bds->newBankIODetaillList("两票制自销付款单-审核",FIdConst::SELF_PAY,$account_1,"主付款账户：对应单号：".$bill['bill_code'],-$money_1)&&
                    $bds->newBankIODetaillList("两票制自销付款单-审核",FIdConst::SELF_PAY,$account_2,"次付款账户：对应单据信息：".$bill['bill_code'],-$money_2)){
                    $account_db->commit();
                    return true;
                }
            }
