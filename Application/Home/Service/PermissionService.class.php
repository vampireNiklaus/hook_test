...
class PermissionService extends PSIBaseService {
	...
	public function permissionByCategory($params) {
		...
		if(empty($roleId)){
            ...
            //获取该角色已获取的权限id
            $role_ids = array();
            $gottenResult = $this->permissionList($roleId);
            foreach( $gottenResult as $v){
                $role_ids[] = $v["id"];
            }

            //比较两个数组，返回差集
            $p_ids = array_diff($ids,$role_ids);

            //根据p_ids查询没有选择的权限,
            if(!empty($p_ids)){
                $map['id'] = array('in',$p_ids);
            }
            if($category != $this->ALL_CATEGORY){
                $map['category'] = $category;
            }

            $data = M('t_permission')->where($map)->select();
            foreach($data as $k => $v){
                $newResult[$k]["id"] = $v["id"];
