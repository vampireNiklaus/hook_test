 ...
 * @author RCG
 */
class SelfManufacturerHuikuanByTwoService extends PSIBaseService {
    private $LOG_CATEGORY = "业务流程信息-回款单信息";

    public function editSelfManufacturerHuikuanByTwo($params){
        //编辑回款单
        if ($this->isNotOnline()) {
            return $this->emptyResult();
        }
        $selfHuikuan_db = D('bill_self_manufacturer_huikuan_by_two');
        $selfHuikuanSub_db = D('bill_self_manufacturer_huikuan_by_two_sub');
...
//            $data['sum_kaipiao_money']=$data['kaipiao_unit_price']*$data['huikuan_num'];
            ...
            //生成系统单号
            $data['bill_code']="CJHK-S".date('YmdHis');
            if($parent_id==''){
...
//                $selfHuikuan_db->where('id='.$params['id'])->setField('had_amount',$huikuan['had_amount']);
     ...
     * 获取未编辑自销回款单列表
     */
    public function listSelfManufacturerHuikuanByTwoUnEdit($params){
        if ($this->isNotOnline()) {
            return $this->emptyResult();
        }
        $selfHuikuan_db = M('bill_self_manufacturer_huikuan_by_two');
        //分页信息
        $page = $params['page'];
        $start = $params['start'];
        $limit = $params['limit'];
...
//            ->join('bill_self_deliver_huikuan_by_two_sub AS sts ON sts.id=shk.deliver_huikuan_bill_id')
...
//            ->join('bill_self_deliver_huikuan_by_two_sub AS sts ON sts.id=shk.deliver_huikuan_bill_id')
    ...
    public function listSelfManufacturerHuikuanByTwoEdit($params){
        if ($this->isNotOnline()) {
            return $this->emptyResult();
        }
        $selfHuikuanSub_db = M('bill_self_manufacturer_huikuan_by_two_sub');
        $page = $params['page'];
        $start = $params['start'];
        $limit = $params['limit'];
...
//            ->join('info_supplier AS isu ON isu.id=shk.supplier_id')
            ...
            ->join('info_bank_account AS iba ON iba.id=shk.huikuan_account')
            ->where("idr.common_name like '%".$params['common_name']."%' AND shk.bill_code like '%".$params['bill_code']."%'")
            ->field('shk.*,idr.common_name,idr.jx,idr.goods_name,idr.guige,idr.manufacturer,
    ...
    public function getHuikuanAmount($id){
        $p_id=M('bill_self_manufacturer_huikuan_by_two_sub')->where('id='.$id)->getField('parent_id');
        $re=M('bill_self_manufacturer_huikuan_by_two')
            ->alias('bst')
            ->where('bst.id='.$p_id)
     ...
     * *
     */
    public function deleteSelfManufacturerHuikuanByTwo($id,$isSub){
        if ($this->isNotOnline()) {
            return $this->notOnlineError();
        }

        $db = M("bill_self_manufacturer_huikuan_by_two");
        $subDb = M("bill_self_manufacturer_huikuan_by_two_sub");
        ...
        if(!$isSub&&$canDelete){
            //实例化商业回款相关模型
            $delSub_db=M('bill_self_deliver_huikuan_by_two_sub');
            $del_db=M('bill_self_deliver_huikuan_by_two');
            //设置对应子商业回款单状态为被退回
            $delSub_db->where('id='.$data['deliver_huikuan_bill_id'])->save(array('status'=>FIdConst::SELF_DELIVER_HUIKUAN_STATUS_BACK));

            //获取对应子商业回款单信息
            $del=$delSub_db->where('id='.$data['deliver_huikuan_bill_id'])->find();
            //获取父商业回款单已开票数
            $hadAmount=$del_db->where('id='.$del['parent_id'])->getField('had_amount');
            //已开票数减去退回的--当前已开票数
            $hadAmountNow=$hadAmount-$del['huikuan_num'];
            //保存当前已开票数
            $del_db->where('id='.$del['parent_id'])->setField('had_amount',$hadAmountNow);
...
//            $db->where('id='.$son_data['parent_id'])->setField('had_amount',$parent_data['had_amount']);
    ...
    public function selfHuikuanStatus($params){
        ...
        $id = $params["id"];
        $type = $params["type"];
        $huikuan_db = M('bill_self_manufacturer_huikuan_by_two');
        $huikuanSub_db = M('bill_self_manufacturer_huikuan_by_two_sub');
        //能修改的条件
    ...
    protected function accountPayMoney($id){
        //实例化模型
        $pay_db=M('bill_self_manufacturer_huikuan_by_two_sub');
