 ...
 * @author RCG
 */
class DirectPayService extends PSIBaseService {
    //获取新添加未确认的业务支付记录
    public function getNewDirectPay($params){
        if ($this->isNotOnline()) {
...
//        import("ORG.Util.Page");
        ...
        $count=$businessPay_db
            ...
            ->field('ds.*,ie.name employee_name ,ie.bank_account as employee_bank_account')
            ->where($where)
            ->count();
...
//            ->page($page,$limit)
    ...
    //添加新的待付款业务支付单
    public function editDirectPay($params){
...
//        $data['pay_month']=substr($sell['sell_date'],0,7);
     ...
     */
    public function deleteDirectPay($list){
        if(count($list)>0){
            $businessPay_db=M('bill_business_pay');
            $dailySell_db=M('bill_daily_sell');
...
//                      $mobile = "18768119456";
    ...
    public function accountPayMoney($id){
        ...
        if($account_can){
            ...
            $data['operate_info'] = $account_db->where("id=".$account)->getField("operate_info").$this->getOperateInfo("直营结算审核资金操作，原有资金：$account_money"."操作金额：$pay_money");
            $account_db->startTrans();
            try{
                if($bds->newBankIODetaillList("直营结算单-审核",FIdConst::BUSINESS_PAY_VERIFY,$account,"付款账户：对应单号：".$bill['bill_code'],-$pay_money)){
    ...
    public function accountReturnMoney($id){
        ...
        if($account>0&&$pay_money>=0){
            ...
            $data_1['operate_info'] = $account_db->where("id=".$account)->getField("operate_info").$this->getOperateInfo("直营结算反审核资金操作，原有资金：$account_money"."操作金额：$pay_money");
            $account_db->startTrans();
            try{
                if($bds->newBankIODetaillList("直营结算单-反审核",FIdConst::BUSINESS_PAY_REVERT_VERIFY,$account,"对应单号：".$bill['bill_code'],$pay_money)){
