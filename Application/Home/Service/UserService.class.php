...
class UserService extends PSIBaseService {
    ...
    public function yeWuSetHospitalAssignList($params) {
        if ($this->isNotOnline()) {
        ...
        if(M('t_user')->where("id='".$userId."'")->getField('all_hospital_view')) {
            $db = M("info_drug2hospital");
            $data = $db
                ->alias('info')
                ->join(" left join info_drug as drug on drug.id = info.drug_id")
                ->field("hospital_id,drug_id,hospital_name,common_name,jx,guige,manufacturer")
                ->page($page,$limit)
                ->select();
            ...
            $data = $db
                ->alias('yewuset')
                ->join(" left join t_user as u on u.id='".$userId."'")
                ...
                ->field("hos.id as hospital_id,drug_id,hospital_name,common_name,jx,guige,manufacturer")
                ->page($page,$limit)
                ->select();
        ...
        return array(
            "totalCount" => $count,
            "yeWuSetHospitalAssignList" => $data
        );
    }



    public function editYeWuSet4HospitalAssign($params){
        if ($this->isNotOnline()) {
            return $this->notOnlineError();
        }

        $userId = $params["user_id"];
        $hospitalAssignIdList = $params["hospitalAssignIdList"];
        $dataOrgList = $params["dataOrgList"];
		$drugIdList = $params["drugIdList"];

        if ($this->isDemo() && $userId == DemoConst::ADMIN_ROLE_ID) {
        ...
        $hid = explode(",", $hospitalAssignIdList);
        $doList = explode(",", $dataOrgList);
        $drugList = explode(",", $drugIdList);

        if ($userId) {
            $db->startTrans();
            try {
                $db->where("user_id='" . $userId . "'")->delete();
                $db->commit();
            } catch (Exception $e) {
                $db->rollback();
...
//		return true;
	...
	public function getAdminUserId() {
		return DemoConst::ADMIN_USER_ID;
	}
	 ...
	 * 业务员身份登陆的时候的获取业务员登录名
	 * */
	public function getLoignUserName4Employee() {
		$user_infos  = M('info_employee')->where('id='.session('loginUserId'))->select();
		$userName = "";
		if(count($user_infos)){
    ...
    /*
     *用户相关的业务设置--获取该登录用户可以获得所有的身份信息
     */
	public function  getLoginUserYeWuSetHospitalLists(){
        $data = [];
        if($this->getLoginUserId() == DemoConst::ADMIN_USER_ID){
            return true;
        }elseif($this->getLoginType() == FIdConst::LOGIN_TYPE_STAFF){
            //公司员工身份登录
            $data = M("t_yewuset_user2hospital")->where("user_id='".$this->getLoginUserId()."'")->getField("hospital_id",true);
        }
        return $data;
    }
	...
	public function doLogin($loginName, $password, $fromDevice,$loginType) {
		if($loginType==FIdConst::LOGIN_TYPE_STAFF){
			...
			if ($user) {
				...
				session("loginType",FIdConst::LOGIN_TYPE_STAFF);
		...
		}else if($loginType==FIdConst::LOGIN_TYPE_EMPLOYEE){
			$sql = "select id from info_employee where client_user_name = '%s' and client_password = '%s' and login_enable = 1";
			$user = M()->query($sql, $loginName, $password);

			if ($user) {
				session("loginUserId", $user[0]["id"]);
				session("loginType", FIdConst::LOGIN_TYPE_EMPLOYEE);
				$bls = new BizlogService();
				$bls->insertBizlog("业务员登录系统，业务员id：".$user[0]["id"]);
				return $this->ok($user[0]["id"]);
			} else {
				return $this->bad("用户名或者密码错误");
			}
	...
	public function users($params) {
		...
		$sql = "select id, login_name,  name, enabled, org_code, gender, birthday, id_card_number, tel,
				    tel02, address, data_org, all_hospital_view
				from t_user
				where org_id = '%s' 
				order by org_code
				limit %d , %d ";
		...
		foreach ( $data as $key => $value ) {
			...
			$result[$key]["all_hospital_view"] = $value["all_hospital_view"];
		}
	...
	public function changeMyPassword($params) {
		...
		if($loginType == FIdConst::LOGIN_TYPE_STAFF){
			...
			// 检验旧密码
			$db = M();
			$sql = "select count(*) as cnt from info_employee where id = '%s' and client_password = '%s' ";
			$data = $db->query($sql, $userId, $oldPassword);
			$cnt = $data[0]["cnt"];
			if ($cnt != 1) {
				return $this->bad("旧密码不正确");
			}
			...
			if (strlen($newPassword) < 6) {
				return $this->bad("密码长度不能小于6位");
			}

			$sql = "select name, client_user_name from info_employee where id = '%s' ";
			$data = $db->query($sql, $userId);
			if (! $data) {
				return $this->bad("要修改密码的用户不存在");
			}
			$loginName = $data[0]["client_user_name"];
			$name = $data[0]["name"];

			$sql = "update info_employee set client_password = '%s' where id = '%s' ";
			$db->execute($sql, $newPassword, $userId);

			$log = "用户(业务员）[登录名 ={$loginName} 姓名 = {$name}]修改了自己的登录密码";
			$bs = new BizlogService();
			$bs->insertBizlog($log, "用户管理");
			return $this->ok();
		}
