 ...
 * @author Baoyu Li
 */
class SMSService extends PSIBaseService {

	public function  sendSMS($mobile,$sell_time,$pay_time,$content){
		$apikey = "2d81f41f386321677b3a042f95420e8a"; //修改为您的apikey(https://www.yunpian.com)登陆官网后获取
...
// 发送模板短信
// 需要对value进行编码
		$data=array('tpl_id'=>'1574262','tpl_value'=>('#sell_time#').'='.urlencode($sell_time).'&'.urlencode('#pay_time#').'='.urlencode($pay_time).'&'.urlencode('#content#').'='.urlencode($content),'apikey'=>$apikey,'mobile'=>$mobile);
		print_r ($data);
