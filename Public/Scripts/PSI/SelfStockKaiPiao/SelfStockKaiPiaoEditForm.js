 ...
 * 自销入库单 - 编辑界面
 */
Ext.define("PSI.SelfStockKaiPiao.SelfStockKaiPiaoEditForm", {
		...
		Ext.apply(me, {
			...
			items : [{
				...
				[
				{
					...
					colspan : 6,
					width:200,
					value : entity == null ? null : entity.get("in_stock_amount"),
					listeners : {
						specialkey : {
							fn : me.onEditSpecialKey,
							scope : me
	...
	onOK : function(thenAdd) {
		...
		if(me.getDirection() == 0){
			if(Ext.getCmp("supplier").getIdValue())
				Ext.getCmp("supplier_id").setValue(Ext.getCmp("supplier").getIdValue());
				}

		f.submit({
			url : PSI.Const.BASE_URL + "Home/SelfStockKaiPiao/editSelfStockKaiPiao",
			method : "POST",
	...
	recountAll:function(){
		...
		var num = Ext.getCmp('stock_amount').getValue();
		if(num&& Ext.getCmp('stock_amount').isValid()){
