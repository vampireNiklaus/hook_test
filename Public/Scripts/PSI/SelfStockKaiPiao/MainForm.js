...
var SELF_STOCK_KAIPIAO_STATUS_2STOCK = "0"; //待入库
var SELF_STOCK_KAIPIAO_STATUS_STOCKED = "3"; //已全部入库
var SELF_STOCK_KAIPIAO_STATUS_2VERIFY = "0"; //子单，待审核
var SELF_STOCK_KAIPIAO_STATUS_VERIFY_PASSED = "1"; //审核通过
var SELF_STOCK_KAIPIAO_STATUS_VERIFY_DENIED = "2"; //审核未通过
var SELF_STOCK_KAIPIAO_STATUS_TAX_BACK = "4"; //税票单被撤回
var defaultDate = new Date("2017-1-1")

Ext.define("PSI.SelfStockKaiPiao.MainForm", {
	extend: "Ext.panel.Panel",

	config: {
		pViewSelfWarehouseInKaiPiaoBill: null,
		pAddSelfWarehouseInKaiPiaoBill: null,
		pEditSelfWarehouseInKaiPiaoBill: null,
		pDeleteSelfWarehouseInKaiPiaoBill: null,
		pImportSelfWarehouseInKaiPiaoBill: null,
		pExportSelfWarehouseInKaiPiaoBill: null,
		pVerifySelfWarehouseInKaiPiaoBill: null,
		pRevertVerifySelfWarehouseInKaiPiaoBill: null
	},
	//页面初始化
		...
		Ext.apply(me, {
			...
			items: [{
				...
				items: [{
					...
					border: 0,
					//获取未编辑入库单
					items: [me.getPViewSelfWarehouseInKaiPiaoBill() == 0 ? null : me.getUnEditedGrid()]
				}, {
					region: "center",
					layout: "fit",
					border: 0,
					//获取已编辑入库单
					items: [me.getPViewSelfWarehouseInKaiPiaoBill() == 0 ? null : me.getEditedGrid()]
				}]
			}]
		});
	...
	getToolbarCmp: function() {
		...
		return [{
			...
			disabled: me.getPEditSelfWarehouseInKaiPiaoBill() == "0",
			id: "buttonEdit"
		}, "-", {
			text: "删除单据",
			iconCls: "PSI-button-delete",
			disabled: me.getPDeleteSelfWarehouseInKaiPiaoBill() == "0",
			scope: me,
			handler: me.onDeleteBill,
			id: "buttonDelete"
		}, "-", {
			text: "审核入库",
			disabled: me.getPVerifySelfWarehouseInKaiPiaoBill() == "0",
			...
			text: "反审核",
			disabled: me.getPRevertVerifySelfWarehouseInKaiPiaoBill() == "0",
			...
			text: "导入自销付款单列表",
			disabled: me.getPImportSelfWarehouseInKaiPiaoBill() == "0",
	...
	getUnEditedGrid: function() {
		...
		var modelName = "PSISelfStockKaiPiaoBill";
		Ext.define(modelName, {
			extend: "Ext.data.Model",
			fields: ["id", "drug_id", 'common_name', 'goods_name', 'jx', 'guige', 'manufacturer', 'jldw',
				'bill_code', "pay_bill_id", 'pay_bill_code', "supplier_id", 'supplier_name',
				"deliver_id", "stock_amount", "need_amount", 'status_str', 'status', 'operate_info', 'buy_date'
			]
		});
		var store = Ext.create("Ext.data.Store", {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/SelfStockKaiPiao/listSelfStockKaiPiaoUnEdit",
				reader: {
					root: 'selfStockKaiPiaoList',
		...
		me.unEditedGrid = Ext.create("Ext.grid.Panel", {
			...
			tbar: [{
				...
				text: "导出待入库列表信息",
				disabled: me.getPExportSelfWarehouseInKaiPiaoBill() == "0",
	...
	getEditedGrid: function() {
		...
		Ext.define(modelName, {
			extend: "Ext.data.Model",
			fields: ["id", "drug_id", 'common_name', 'goods_name', 'jx', 'guige', 'manufacturer', 'jldw',
				'bill_code', 'buy_bill_code', 'pay_bill_code', "supplier_id", 'supplier_name', "stock_amount", "remain_amount","buy_date",
				'status', 'status_str', 'batch_num', 'validity', 'parent_id', 'note', 'operate_info', 'instock_date', 'kaipiao_unit_price'
			]
		});
		var store = Ext.create("Ext.data.Store", {
			autoLoad: false,
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/SelfStockKaiPiao/listSelfStockKaiPiaoEdit",
				reader: {
					root: 'selfStockKaiPiaoList',
					totalProperty: 'totalCount'
				}
				...
				listeners: {
					...
					beforeshow: function updateTipBody(tip) {
						...
						tip.update(
							...
							"应出库总数：<b style='color:red'>" + re.get('stock_amount') + "</b></br>" +
							"未出库总数：<b style='color:blue'>" + re.get('remain_amount') + "</b></br>" +
	...
	onEditBill: function() {
		...
		switch (bill.get('status')) {
			case SELF_STOCK_KAIPIAO_STATUS_VERIFY_PASSED:
				PSI.MsgBox.showInfo("已审核出库，无法编辑");
				return;
			case SELF_STOCK_KAIPIAO_STATUS_STOCKED:
		...
		if (!flag) {
			//异步获取总量和剩余入库量
			Ext.Ajax.request({
				url: PSI.Const.BASE_URL + "Home/SelfStockKaiPiao/getStockAmount",
				method: 'POST',
				params: {
					id: bill.get('id')
				},
				success: function(response) {
					...
					bill.data.in_stock_amount = bill.data.stock_amount;
					bill.data.stock_amount = re['stock_amount'];
					bill.data.need_amount = re['need_amount'];
					var form = Ext.create("PSI.SelfStockKaiPiao.SelfStockKaiPiaoEditForm", {
						parentForm: me,
						entity: bill,
						direction: 0, //编辑提交入库单据
					});
					form.show();
				}
			});
	...
	onDeleteBill: function() {
		...
		PSI.MsgBox.confirm(info, function() {
			...
			el.mask("正在删除中...");
			Ext.Ajax.request({
				url: PSI.Const.BASE_URL + "Home/SelfStockKaiPiao/deleteSelfStockKaiPiao",
				method: "POST",
				params: {
					id: bill.get("id"),
	...
	onCommit: function() {
		...
		if (bill.get("status") == SELF_STOCK_KAIPIAO_STATUS_VERIFY_PASSED) {
			PSI.MsgBox.showInfo("该出库单已经审核过，成功入库了");
			return;
		}

		var info = "请确认入库单: <span style='color:red'>系统单号：" + bill.get("bill_code") + "，药品：" + bill.get("common_name") + "，开票公司：" + bill.get("supplier_name") + "</span> 审核通过?";
	...
	onCommitReturn: function() {
		...
		var info = "确认要反审核该入开票公司单: <span style='color:red'>系统单号：" + bill.get("bill_code") + "，药品：" + bill.get("common_name") + "</span> ?";
		PSI.MsgBox.confirm(info, function() {
	...
	verifyRequest: function(bill, type) {
		...
		el.mask("正在提交中...");
		Ext.Ajax.request({
			url: PSI.Const.BASE_URL + "Home/SelfStockKaiPiao/selfStockStatus",
			method: "POST",
			params: {
				id: id,
				type: type
