 ...
 * @author Baoyu Li
 */
Ext.define("PSI.BillingTypes.MainForm", {
	extend: "Ext.panel.Panel",

	config: {
		pAddBillTypes: null,
		pEditBillTypes: null,
		pDeleteBillTypes: null
	},
	...
	initComponent: function() {
		...
		var store = Ext.create("Ext.data.Store", {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/BillingTypes/listTypes",
				reader: {
					root: 'typesList',
		...
		Ext.apply(me, {
			...
			tbar: [{
				text: "新增类别",
				disabled: me.getPAddBillTypes() == "0",
				iconCls: "PSI-button-add",
				handler: me.onAddTypes,
				scope: me
			}, {
				text: "编辑类别",
				disabled: me.getPEditBillTypes() == "0",
				iconCls: "PSI-button-edit",
				handler: me.onEditTypes,
				scope: me
			}, {
				text: "删除类别",
				disabled: me.getPDeleteBillTypes() == "0",
	 ...
	 */
	onAddTypes: function() {
		var form = Ext.create("PSI.BillingTypes.BillingTypesEditForm", {
			parentForm: this
		});
		form.show();
	},
	...
	onEditTypes: function() {
		var me = this;
		if (me.getPEditBillTypes() == "0") {
			PSI.MsgBox.showInfo("没有编辑条目权限");
			return;
		}
	...
	onDeleteTypes: function() {
		...
		var info = "请确认是否删除资金条目: <span style='color:red'>" + types.get("name") + " " + types.get("code") + "</span>";

		PSI.MsgBox.confirm(info, function() {
			var el = Ext.getBody();
			el.mask("正在删除中...");
			Ext.Ajax.request({
				url: PSI.Const.BASE_URL + "Home/BillingTypes/deleteType",
				method: "POST",
				params: {
					id: types.get("id")
				},
