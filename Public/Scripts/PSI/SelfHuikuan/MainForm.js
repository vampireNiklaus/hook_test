...
Ext.define("PSI.SelfHuikuan.MainForm", {
	extend: "Ext.panel.Panel",
	config: {
		pAddSelfHuikuanBill: null,
		pEditSelfHuikuanBill: null,
		pDeleteSelfHuikuanBill: null,
		pImportSelfHuikuanBill: null,
		pExportSelfHuikuanBill: null,
		pVerifySelfHuikuanBill: null,
		pRevertVerifySelfHuikuanBill: null
	},
	//页面初始化
	...
	getToolbarCmp: function() {
		...
		return [{
			...
			scope: me,
			disabled: me.getPEditSelfHuikuanBill() == "0",
			handler: me.onEditBill,
			id: "buttonEdit"
		}, "-", {
			text: "删除回款单",
			disabled: me.getPDeleteSelfHuikuanBill() == "0",
			...
			text: "审核",
			disabled: me.getPVerifySelfHuikuanBill() == "0",
			...
			scope: me,
			disabled: me.getPRevertVerifySelfHuikuanBill() == "0",
			handler: me.onCommitReturn,
			id: "buttonRevertVerify"
		}, "-", {
			text: "导入自销回款单列表",
			disabled: me.getPImportSelfHuikuanBill() == "0",
	...
	getUnEditedGrid: function() {
		...
		var modelName = "PSISelfHuikuanBill";
		Ext.define(modelName, {
			extend: "Ext.data.Model",
			fields: ["id", "drug_id", 'common_name', 'goods_name', 'jx', 'guige', 'manufacturer',
				'bill_code', "tax_bill_id", 'tax_sub_bill_code', "supplier_id", 'supplier_name',
		...
		var store = Ext.create("Ext.data.Store", {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/SelfHuikuan/listSelfHuikuanUnEdit",
				reader: {
		...
		me.unEditedGrid = Ext.create("Ext.grid.Panel", {
			...
			tbar: [{
				...
				text: "导出未编辑回款单信息",
				disabled: me.getPExportSelfHuikuanBill() == "0",
	...
	getEditedGrid: function() {
		...
		Ext.define(modelName, {
			...
			fields: ["id", "drug_id", 'common_name', 'goods_name', 'jx', 'guige', 'manufacturer',
				...
				'huikuan_num', 'sum_kaipiao_money', 'huikuan_account', 'huikuan_account_name',
		...
		var store = Ext.create("Ext.data.Store", {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/SelfHuikuan/listSelfHuikuanEdit",
				reader: {
					root: 'selfHuikuanList',
					totalProperty: 'totalCount'
				}
	...
	onEditBill: function() {
		...
		if (!isParent) {
			//异步获取总量和剩余入库量
			Ext.Ajax.request({
				url: PSI.Const.BASE_URL + "Home/SelfHuikuan/getHuikuanAmount",
				method: 'POST',
				params: {
					id: bill.get('id')
				},
				success: function(response) {
					...
					var form = Ext.create("PSI.SelfHuikuan.SelfHuikuanEditForm", {
						parentForm: me,
						entity: bill
					});
					form.show();
	...
	onDeleteBill: function() {
		...
		PSI.MsgBox.confirm(info, function() {
			...
			el.mask("正在删除中...");
			Ext.Ajax.request({
				url: PSI.Const.BASE_URL + "Home/SelfHuikuan/deleteSelfHuikuan",
				method: "POST",
				params: {
					id: bill.get("id"),
	...
	verifyRequest: function(bill, type) {
		...
		el.mask("正在提交中...");
		Ext.Ajax.request({
			url: PSI.Const.BASE_URL + "Home/SelfHuikuan/selfHuikuanStatus",
			method: "POST",
			params: {
				id: id,
				type: type
