 ...
 * 自销回款单 - 编辑界面
 */
Ext.define("PSI.SelfManufacturerHuikuanByTwo.SelfManufacturerHuikuanByTwoEditForm", {
		...
		Ext.apply(me, {
			...
			items: [{
				...
				items: [{
					...
					colspan: 4,
					name: "huikuan_money",
					value: entity == null ? null : entity.get("huikuan_money"),
                    listeners: {
                        specialkey: {
                            fn: me.onEditSpecialKey,
                            scope: me
	...
	onOK: function(thenAdd) {
		...
		if (huikuan_num - need > 0) {
			...
			f.submit({
				url: PSI.Const.BASE_URL + "Home/SelfManufacturerHuikuanByTwo/editSelfManufacturerHuikuanByTwo",
				method: "POST",
	...
	recountAll: function() {
		var config = {
			'kaipiao_unit_price': 'huikuan_money',
		};
		//获取数量
		...
		if(huikuan_way == 2){
			//回款方式为商业公司预付，就不进行计算
            Ext.getCmp('huikuan_money').setValue('');
		}else{
