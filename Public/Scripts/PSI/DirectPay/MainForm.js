...
var records = {}
Ext.define("PSI.DirectPay.MainForm", {
	extend: "Ext.panel.Panel",

	config: {
		pAddDirectPay: null,
		pEditDirectPay: null,
		pDeleteDirectPay: null,
		pImportDirectPay: null,
		pExportDirectPay: null,
		pVerifyDirectPay: null,
		pRevertVerifyDirectPay: null,
		pViewDirectPayDetail: null,
		pDirectPaySearch: null,
	},
	...
	initComponent: function() {
		var me = this;
		me.newDirectPayGrid = null;
		me.allDirectPayGrid = null;

		me.modelName = "PSINewDirectPay";
		...
		Ext.apply(me, {
			...
			tbar: [{
				text: "新增直营结算条目",
				disabled: me.getPAddDirectPay() == "0",
				iconCls: "PSI-button-add",
				handler: me.onAddDirectPayItem,
				scope: me
			}, {
				text: "编辑直营结算条目",
				disabled: me.getPEditDirectPay() == "0",
				iconCls: "PSI-button-edit",
				handler: me.onEditDirectPay,
				scope: me
			}, {
				text: "删除直营结算条目",
				disabled: me.getPDeleteDirectPay() == "0",
				iconCls: "PSI-button-delete",
				handler: me.onDeleteDirectPay,
				scope: me
			}, "-", {
				text: "审核",
				disabled: me.getPVerifyDirectPay() == "0",
				...
				disabled: me.getPRevertVerifyDirectPay() == "0",
				scope: me,
				...
				text: "导出直营结算信息",
				disabled: me.getPExportDirectPay() == 0,
				iconCls: "PSI-button-excelexport",
				handler: me.onExportDirectPay,
				scope: me
			}, "-", {
				text: "关闭",
			...
			items: [me.getPDirectPaySearch() == "0" ? {} : {
				region: "north",
				border: 0,
				height: 60,
				title: "查询条件",
				...
				layout: "fit",
				border: 0,
				items: me.getNewDirectPayGrid()
			}]
		});

		me.callParent(arguments);

		me.queryTotalDirectPayCount();
		me.freshDirectPayGrid();
		me.__queryEditNameList = ["editQueryName",
			"editQueryType"
		];
	},
	//获取新添加未确认的直营结算记录
	getNewDirectPayGrid: function() {
		var me = this;
		...
		if (me.__newDirectPayGrid) {
			return me.__newDirectPayGrid;
		}
		var modelName = "PSIDirectPayModel";
		...
		var store = Ext.create("Ext.data.Store", {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/DirectPay/getNewDirectPay",
				reader: {
					root: 'all_data',
		...
		// 		//enableKeyNav:false
		// 	});
		me.__newDirectPayGrid = Ext.create("Ext.grid.Panel", {
			viewConfig: {
			...
			listeners: {
				...
				selectionchange: function(view, record, item, index, e) {
                    ...
                    me.__newDirectPayGrid.update();
				}
			}
		});

		var summaryColumns = me.__newDirectPayGrid.columns;
		...
		me.__newDirectPayGrid.view.on('expandBody', function(rowNode, record, expandRow, eOpts) {
			if (me.getPViewDirectPayDetail() == "0") {
		...
		me.__newDirectPayGrid.view.on('collapsebody', function(rowNode, record, expandRow, eOpts) {
		...
		me.newDirectPayGrid = me.__newDirectPayGrid;
		return me.__newDirectPayGrid;
	},
	...
	displayInnerGrid: function(renderId) {
		...
		var store = Ext.create('Ext.data.Store', {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/DirectPay/getDailySellById",
				extraParams: {
					'id': renderId
				},
				reader: {
	...
	gotoDirectPayGridRecord: function() {
		var me = this;
		var grid = me.DirectPayGrid;
	 ...
	 * 新增DirectPay
	 */
	onAddDirectPay: function() {
		if (this.getRegionGrid().getStore().getCount() == 0) {
			PSI.MsgBox.showInfo("没有选择区域，请先选择区域");
			return;
		}
		var form = Ext.create("PSI.DirectPay.DirectPayEditForm", {
			parentForm: this
		});
		form.show();
	},
	 ...
	 * 编辑DirectPay
	 */
	onEditDirectPay: function() {
		var me = this;
		if (me.getPEditDirectPay() == "0") {
			PSI.MsgBox.showInfo("没有编辑权限，无法编辑！");
			return;
		}
		var item = this.newDirectPayGrid.getSelectionModel().getSelection();;
		if (item == null || item.length != 1) {
	 ...
	 * 删除直营结算
	 */
	onDeleteDirectPay: function() {
		var me = this;
		var item = me.newDirectPayGrid.getSelectionModel().getSelection();

		if (item == null || item.length == 0) {
			PSI.MsgBox.showInfo("请选择要删除的直营结算");
			return;
		}


		var info = "确认要删除选中的" + item.length + "条直营结算单吗？";
		...
		PSI.MsgBox.confirm(info, function() {
			...
			el.mask("正在删除中...");
			Ext.Ajax.request({
				url: PSI.Const.BASE_URL + "Home/DirectPay/deleteDirectPay",
				method: "POST",
				params: params,
	 ...
	 * 导出直营结算信息
	 */
	onExportDirectPay: function() {
		//var url = PSI.Const.BASE_URL + "Home/DirectPay/exportDirectPay";
		//window.open(url);
		var me = this;
		var config = {
			store: me.__newDirectPayGrid.getStore(),
			title: "直营结算直营结算单表格"
		};
		//var tab=tabPanel.getActiveTab();//当前活动状态的Panel
		ExportExcel(me.__newDirectPayGrid, config); //调用导出函数
	},
	...
	getExportData: function() {
		...
		var store = this.newDirectPayGrid.getStore();
	 ...
	 * 刷新直营结算Grid
	 */
	freshDirectPayGrid: function(id) {
		var me = this;
		var grid = me.newDirectPayGrid;
		var store = grid.getStore();
		store.removeAll();
		store.load();
	},
	...
	onRegionGridSelect: function() {
		var me = this;
		me.DirectPayGrid.getStore().currentPage = 1;
		me.freshDirectPayGrid();
	},
	queryTotalDirectPayCount: function() {
		var me = this;
		// Ext.Ajax.request({
		// 	url : PSI.Const.BASE_URL + "Home/DirectPay/getTotalDirectPayCount",
		// 	method : "POST",
		...
		// 			Ext.getCmp("fieldTotalDirectPayCount").setValue("共有直营结算"
		// 				+ data.cnt + "家");
		// 		}
		// 	}
		// });
	...
	onCommit: function() {
		...
		var list = [];
		if (item == null || item.length == 0) {
			PSI.MsgBox.showInfo("没有选择要操作的直营结算单");
			return;
		}
		...
		if (list.length == 0) {
			PSI.MsgBox.showInfo("该直营结算单已通过审核！");
			return;
		}

		var info = "确认选中的直营结算单审核通过?（自动过滤已审核）";
	...
	onCommitReturn: function() {
		...
		if (list.length == 0) {
			PSI.MsgBox.showInfo("该直营结算单无法进行此操作");
			return;
		}

		var info = "确认要反审核该直营结算单？";
		PSI.MsgBox.confirm(info, function() {
	...
	verifyRequest: function(list, type) {
		...
		el.mask("正在提交中...");
		Ext.Ajax.request({
			url: PSI.Const.BASE_URL + "Home/DirectPay/businessPayStatus",
			method: "POST",
			params: {
				list: list,
				type: type,
			...
			callback: function(options, success, response) {
				...
				// me.newDirectPayGrid.deselectAll();
			}
		});
	},
