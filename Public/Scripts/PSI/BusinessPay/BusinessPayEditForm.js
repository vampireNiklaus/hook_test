 ...
 * @author rcg
 */
Ext.define("PSI.BusinessPay.BusinessPayEditForm", {
    ...
    initComponent: function () {
        ...
        if (status !== 1) {
            var buttons = [];
            buttons.push({
                text: entity == null ? "添加到待付款业务支付单" : '保存修改',
                formBind: true,
        ...
        Ext.apply(me, {
            title: entity == null ? "新增业务支付单" : "编辑业务支付单",
            modal: true,
            resizable: true,
            maximizable: true,
            maximized: true,
                        ...
                        items: me.adding == true ? [
                                {
                                    ...
                                    fieldLabel: "支付日期",
                                    margin: "5, 0, 0, 0",
                                    allowBlank: me.adding == true ? true : false,
                                    xtype: "datefield",
                                    format: "Y-m-d",
                                    value: new Date(),
                                    listeners: {
                                ...
                                //width:300,
                                xtype: "psi_bank_account_field",
                                value: entity.get('pay_account_name') == null||entity.get('pay_account_num') == null||entity.get('pay_account_id') == 0 ? null : entity.get('pay_account_name') + "，" + "卡号：" + entity.get('pay_account_num'),
                                listeners: {
                                    change: function () {
            ...
            listeners: {
                ...
                close: {
                    fn: function () {
                        me.getParentForm().freshBusinessPayGrid();
                    },
                    scope: me
                }
            }
    ...
    onOk: function () {
        ...
        // if (account != null && account != '' && bill_date != null && bill_date != '' && pay_month != null && pay_month != '') {
            ...
            if (params.select_0) { //存在被选择的条目
                ...
                el.mask('正在添加...');
                f.submit({
                    url: PSI.Const.BASE_URL + "Home/BusinessPay/editBusinessPay",
                    method: "POST",
                    params: params,
