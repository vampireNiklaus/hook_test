 ...
 * @author rcg
 */
Ext.define("PSI.InvestPay.InvestPayEditForm", {
	...
	initComponent: function() {
		...
		if (status !== 1) {
			var buttons = [];
			buttons.push({
				text: entity == null ? "添加到待付款招商结算单" : '保存修改',
				formBind: true,
		...
		Ext.apply(me, {
			title: entity == null ? "新增招商结算单" : "编辑招商结算单",
			modal: true,
			resizable: true,
			maximizable: true,
			maximized: true,
				...
				items: [{
					...
					items: [{
						...
						items: me.adding == true ? [{
							...
							xtype: 'displayfield',
							margin: '5px',
							value: entity == null ? "" : entity.get('agent_name'),
							renderer: function(v) {
			...
			listeners: {
				...
				close: {
					fn: function() {
						me.getParentForm().freshInvestPayGrid();
					},
					scope: me
				}
			}
	...
	getDailySellGrid: function() {
		...
		Ext.define(modelName, {
			...
			fields: ["agent_id", 'pay_sum_money', "agent_name",'now_date']
		});
		var store = Ext.create("Ext.data.Store", {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/ProtocolManage/getAgentList4AddInvestPayItem",
				reader: {
					root: 'agentList',
	...
	getDailySellSubGrid: function() {
		...
		var url = me.adding ? "Home/DailySell/getDailySellDetailByAgent" : "Home/DailySell/getEditDailySellDetailByAgent";
	...
	getQueryParam: function() {
		...
		var agent_id = Ext.getCmp("editQueryAgent").getIdValue();
		if (agent_id) {
	...
	onOk: function() {
		...
		//新增和编辑的时候进行不同判断
        if(me.entity){
        	var agent_id = me.entity.data.agent_id;
		}else{
            var item = me.dailySellGrid.getSelectionModel().getSelection();
            var agent_id = item[0].get('agent_id');

            if (agent_id == null || agent_id == '') {
                PSI.MsgBox.showInfo("请先选择要查询的代理商");
                return;
            }
		}
		...
		if (account != null && account != '' && bill_date != null && bill_date != '' && pay_month != null && pay_month != '') {
			...
			if (params.select_0) { //存在被选择的条目
				...
				el.mask('正在添加...');
				f.submit({
					url: PSI.Const.BASE_URL + "Home/InvestPay/editInvestPay",
					method: "POST",
					params: params,
