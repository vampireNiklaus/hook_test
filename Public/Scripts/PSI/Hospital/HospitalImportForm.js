 ...
 * @author Baoyu Li
 */
Ext.define("PSI.Hospital.HospitalImportForm", {
		...
		Ext.apply(me, {
			...
			items: [{
				...
				items: [{
						...
						html: "<a href=../Uploads/Drug/DrugModelFile.xls ><h4>医院信息导入模板下载</h4></a>",
						border: 0
					}
					//,{
	...
	onUploadExcel: function() {
		...
		el.mask('正在导入...');
		f.submit({
			url: PSI.Const.BASE_URL + "Home/Hospital/importHospitalExcelList",
			method: "POST",
			success: function(form, action) {
				el.unmask();
                me.focus();
				me.close();
                var form = Ext.create("PSI.Hospital.HospitalTempForm", {
                    parentForm: this
                });
                form.show();
