...
Ext.define("PSI.User.MainForm", {
    ...
    initComponent: function() {
        ...
        Ext.define("PSIUser", {
            ...
            fields: ["id", "loginName", "name", "enabled", "orgCode",
                ...
                "tel02", "address", "dataOrg","all_hospital_view"
            ]
        });
        ...
        var grid = Ext.create("Ext.grid.Panel", {
            ...
            columns: [Ext.create("Ext.grid.RowNumberer", {
                ...
                header: "是否允许查看所有医院",
                dataIndex: "all_hospital_view",
                menuDisabled: false,
                sortable: false,
                renderer: function(value) {
                    return value == 1 ? "允许查看" : "<span style='color:red'>禁止查看</span>";
                }
            }, {
                header: "性别",
                dataIndex: "gender",
        ...
        Ext.apply(me, {
            ...
            tbar: [{
                ...
                disabled: me.getPEditUser() == "0",
                iconCls: "PSI-button-edit-user",
                handler: me.onEditUserYeWuSetting,
                scope: me
            }, "-", "-", {
                text: "帮助",
                iconCls: "PSI-help",
                handler: function() {
                    window.open(PSI.Const.BASE_URL + "/Home/Help/index?t=user");
                }
            }, "-", {
                text: "关闭",
    ...
    onDeleteOrg: function() {
        ...
        var info = "请确认是否删除组织机构 <span style='color:red'>" + org.fullName + "</span> ?";
     ...
     * 修改用户业务区域
     */
    onEditUserYeWuSetting: function() {
        var item = this.grid.getSelectionModel().getSelection();
        if (item === null || item.length !== 1) {
            PSI.MsgBox.showInfo("请选择要修改的用户");
            return;
        }

        var user = item[0].getData();
        var editFrom = Ext.create("PSI.User.EditUserYeWuSettingForm", {
            entity: user
        });
        editFrom.show();
    },
    ...
    onDeleteUser: function() {
        ...
        var info = "请确认是否删除用户 <span style='color:red'>" + user.name + "</span> ?";
        PSI.MsgBox.confirm(info, funcConfirm);
    },
