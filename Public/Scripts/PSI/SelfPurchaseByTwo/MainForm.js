 ...
 * @author RCG
 */
Ext.define("PSI.SelfPurchaseByTwo.MainForm", {
	extend: "Ext.panel.Panel",

	config: {
		pAddSelfPurchaseByTwoBill: null,
		pEditSelfPurchaseByTwoBill: null,
		pDeleteSelfPurchaseByTwoBill: null,
		pImportSelfPurchaseByTwoBill: null,
		pExportSelfPurchaseByTwoBill: null,
		pVerifySelfPurchaseByTwoBill: null,
		pRevertVerifySelfPurchaseByTwoBill: null
	},
	...
	initComponent: function() {
		var me = this;
		var modelName = "PSISelfPurchaseByTwo";
		Ext.define(modelName, {
			extend: "Ext.data.Model",
			fields: ["id", "bill_code", "drug_id", "common_name", 'goods_name', 'jx', 'guige', 'jldw',
				'manufacturer',
				"buy_amount", "base_price", "sum_pay", "tax_unit_price", "huikuan_pay","huikuan_way",
				"kaipiao_unit_price", "buy_date", "kaidan_date", "kaidan_bill_code",
				"note", "kaidan_ren", "verified", "verifier_id", "status", "status_str", "in_use", "create_time", "huikuan_deadline","operate_info"
			]
		});
		...
		var store = Ext.create("Ext.data.Store", {
			...
			proxy: {
				...
				url: PSI.Const.BASE_URL + "Home/SelfPurchaseByTwo/listSelfPurchaseByTwo",
				reader: {
					type: 'json',
				...
				listeners: {
					...
					beforeshow: function updateTipBody(tip) {
						...
						tip.update(
							...
							"<span style='margin-right: 20px'>买货单价：<b style='color:red'>" + re.get('base_price') +
							"</b></span>付款金额：<b style='color:red'>" + re.get('sum_pay') + "</b></br>" +
							"<span style='margin-right: 20px'>开票单价：<b style='color:red'>" + re.get('kaipiao_unit_price') +
							"</b></span>回款金额：<b style='color:red'>" + re.get('huikuan_pay') + "</b></br>" +
							"<span style='margin-right: 20px'>单品税价：<b style='color:red'>" + re.get('tax_unit_price') +
		...
		Ext.apply(me, {
			...
			tbar: [{
				text: "新增自销采购单",
				disabled: me.getPAddSelfPurchaseByTwoBill() == "0",
				iconCls: "PSI-button-add",
				handler: me.onAddSelfPurchaseByTwo,
				scope: me,
				id: 'buttonAdd',
			}, {
				text: "编辑自销采购单",
				disabled: me.getPEditSelfPurchaseByTwoBill() == "0",
				iconCls: "PSI-button-edit",
				handler: me.onEditSelfPurchaseByTwo,
				scope: me,
				id: 'buttonEdit',
			}, {
				text: "删除自销采购单",
				disabled: me.getPDeleteSelfPurchaseByTwoBill() == "0",
				iconCls: "PSI-button-delete",
				handler: me.onDeleteSelfPurchaseByTwo,
				scope: me,
				id: 'buttonDelete',
			}, "-", {
				text: "审核",
				iconCls: "PSI-button-verify",
				disabled: me.getPVerifySelfPurchaseByTwoBill() == "0",
				scope: me,
				handler: me.onCommit,
				id: "buttonVerify"
			}, {
				...
				disabled: me.getPRevertVerifySelfPurchaseByTwoBill() == "0",
				scope: me,
				handler: me.onCommitReturn,
				id: "buttonRevertVerify"
			}, "-", {
				text: "导入自销采购单列表",
				disabled: me.getPImportSelfPurchaseByTwoBill() == "0",
				iconCls: "PSI-button-excelimport",
				handler: me.onImportSelfPurchaseByTwo,
				scope: me
			}, {
				text: "导出自销采购单信息",
				disabled: me.getPExportSelfPurchaseByTwoBill() == "0",
				iconCls: "PSI-button-excelexport",
				handler: me.onExportSelfPurchaseByTwo,
				scope: me
			}, "-", {
				text: "帮助",
				iconCls: "PSI-help",
		...
		me.refreshSelfPurchaseByTwoGrid();
		me.__queryEditNameList = ["bill_code", "common_name"];
	},
	//页面刷新，传入页码的话就跳到指定页码，不传的话就是刷新当前页
	refreshSelfPurchaseByTwoGrid: function(currentPage) {
		var me = this;
	 ...
	 * 新增自销采购单
	 */
	onAddSelfPurchaseByTwo: function() {
		var form = Ext.create("PSI.SelfPurchaseByTwo.SelfPurchaseByTwoEditForm", {
			parentForm: this
		});
	 ...
	 * 编辑自销采购单
	 */
	onEditSelfPurchaseByTwo: function() {
		var me = this;
	 ...
	 * 删除自销采购单
	 */
	onDeleteSelfPurchaseByTwo: function() {
		var me = this;
		...
		PSI.MsgBox.confirm(info, function() {
			...
			el.mask("正在删除中...");
			Ext.Ajax.request({
				url: PSI.Const.BASE_URL + "Home/SelfPurchaseByTwo/deleteSelfPurchaseByTwo",
				method: "POST",
				params: {
	 ...
	 * 导入自销采购单信息
	 */
	onImportSelfPurchaseByTwo: function() {
	 ...
	 * 导出自销采购单信息
	 */
	onEportSelfPurchaseByTwo: function() {
		//var form = Ext.create("PSI.Goods.GoodsImportForm", {
		//	parentForm : this
		//});
		//
	...
	verifyRequest: function(bill, type) {
		...
		el.mask("正在提交中...");
		Ext.Ajax.request({
			url: PSI.Const.BASE_URL + "Home/SelfPurchaseByTwo/selfPurchaseStatus",
			method: "POST",
			params: {
				id: id,
				type: type
	...
	onQuery: function() {
		var me = this;
		me.refreshSelfPurchaseByTwoGrid(1);
	},
	...
	//导出表格
	onExportSelfPurchaseByTwo: function() {
		var grid = this.selfPurchaseGrid;
		var config = {
			store: grid.getStore(),
			title: "自销采购单"
